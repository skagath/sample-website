    ...
    post {
        ...
        failure {
            script {
                ...
                def fullConsoleLog = currentBuild.rawBuild.getLogReader().text

                // Split the log into lines and take the last 50
                def logLines = fullConsoleLog.readLines().takeRight(50).join('\n')

                // Send failure message with log snippet to Slack
                slackSend channel: "${SLACK_CHANNEL_NAME}",
                          color: 'danger',
